...
let read_ast path =
  In_channel.with_file path ~f:(fun in_channel ->
      ...
      ignore
      @@ In_channel.input_buffer in_channel ~len:Misc.Magic_number.magic_length
           buf;
  ...
  | Some file when file_exists file -> (
      ...
      let path =
        Option.map
          ...
          cmt_file.cmt_sourcefile
      in
      match path with
      | Some path when file_exists path ->
          ignore
          @@ Hashtbl.add map ~key:module_ ~data:(path, lazy (read_ast path))
      | _ -> ())
  | _ -> ()
...
let iter_spec map (sexp : Dune_spec.t) =
  match sexp with
  | Dune_spec.Executables ls -> iter_executable map ls
  | Dune_spec.Library ls -> iter_library map ls
